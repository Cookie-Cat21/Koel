...
| `koel` unit coverage | ✅ **100%** (wave 16 milestone) |
...
- `pdf_url` + `disclosure_briefs` schema; `koel/briefs/` stub (`AI_BRIEFS_ENABLED=0`).
...
- Gemini brief provider stub (`koel/briefs/provider.py`).
...
- PDF text extract (`koel/briefs/extract.py`) + worker drain.
...
| SHA | Commit |
|---|---|
| `d46e3ea` | style(wave9): ruff format koel and tests |
...
- `ruff format` over `koel/` and related tests (style-only).
...
- `koel/scenarios/` stub gated by `AI_SCENARIOS_ENABLED=0` with NFA guardrails (reject buy/sell language); no LLM wiring.
...
- Health + circuit breaker coverage to 100% line/branch; config `_float` + migrate `__main__` coverage; `koel.__main__` remaining branches.
...
- **Milestone:** `pytest --cov=koel` → **TOTAL 3427 stmts / 0 miss / 100%** across every `koel` module (adapters, bot, briefs, circuit, config, domain, health, migrate, notify, poller, rules, scenarios, storage, `__main__`).
...
- `web/README.md` — CSRF section: non-HttpOnly `koel_csrf`, `X-CSRF-Token` on mutating `/api/v1/*`, session validated before CSRF (`401` vs `400 csrf_failed`), pointer to `scripts/factory/test_csrf_contract.md`.
...
- [W87_ADVERSARIAL.md](W87_ADVERSARIAL.md) — CLEAN; `koel/rules.py` has no `datetime.now`; ±5m/±1h data-stamp injection still gates correctly.
...
- Rule bool numeric coercion harden in `koel/domain.py` with `tests/test_wave91_rules_medium_bugs.py`.
- CLI/migrate arg soft-accept guards in `koel/__main__.py` / `koel/migrate.py` with `tests/test_wave91_config_medium_bugs.py`.
- Disclosure watermark conflict harden in `koel/storage.py` with `tests/test_wave91_storage_medium_bugs.py`.
- Health poller merge harden in `web/src/app/api/v1/health/route.ts`.
- Alert lookup failure NFA harden in `koel/bot.py` with `tests/test_wave91_bot_medium_bugs.py`.
- CSE numeric payload bool rejection in `koel/adapters/cse.py` with `tests/test_wave91_cse_medium_bugs.py`.
- Brief prompt delimiter harden in `koel/briefs/provider.py` / `koel/briefs/__init__.py` with `tests/test_wave91_briefs_medium_bugs.py`.
