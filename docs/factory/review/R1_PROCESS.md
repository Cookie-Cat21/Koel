...
| **Cross-lane API surface** | DASH owns “dash API”; CORE owns `chime/`. Concurrent CORE+DASH waves can both edit the same route/handler. | Explicit ownership: dash API paths listed in plan; CORE may not touch them in same wave (and vice versa). |
