...
11. **Forbid a second domain layer in `web/`.** Prefer calling shared Python semantics (subprocess/service) *or* a thin typed SQL module that mirrors `koel.storage` method-for-method. Ad-hoc Next SQL will drift on unique-active, armed, and unwatch side effects.
