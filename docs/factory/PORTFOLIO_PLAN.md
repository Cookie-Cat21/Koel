...
**Near-term (active):** [KOEL_HORIZON.md](KOEL_HORIZON.md) — Quiverly `repo_score` **2K–3K** (midpoint 2500).  
...
**Estimated Quiverly-only proper ceiling:** hundreds–low thousands. Operating band **2K–3K**: [KOEL_HORIZON.md](KOEL_HORIZON.md).
...
3. Register in `docs/factory/PORTFOLIO_NODES.json`:
   ```json
   { "repo": "org/koel", "path": ".", "score_file": "docs/factory/SCOREBOARD.json" }
   ```
4. Meta job (later): `scripts/factory/portfolio_sum.py` reads all nodes → `portfolio_score`.

**Until other repos exist:** Quiverly runs solo; `portfolio_score == koel.repo_score`.
...
Climb toward **2K–3K Quiverly `repo_score` first** — product truth and fuel map in [KOEL_HORIZON.md](KOEL_HORIZON.md).
