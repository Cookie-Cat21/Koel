...
- **Where:** `koel/poller.py:144–167`, `koel/storage.py:163–191`  
...
- **Where:** `koel/poller.py:122–131` (disarm loop before `_claim_and_send`)  
...
- **Where:** `koel/bot.py` (handlers); `koel/storage.py:260–309` (create-only)  
...
- **Where:** `koel/poller.py:90–106`, `koel/bot.py:95–112`, `koel/storage.py:222–235`  
...
- **Severity:** critical  
- **Score:** 10  
- **Failure scenario:** Two `python -m koel poller` (or poller + both) share one DB. Both insert a new `price_snapshots` row for the same tick (ids 10 and 11). Both see `previous.price` below threshold and current above. `event_key` is `price:{rule_id}:{snapshot_id}` → **two distinct keys** → two claims → two Telegrams for one cross. `ON CONFLICT (rule_id, event_key)` does not help.  
- **Where:** `koel/rules.py:33–35`, `koel/poller.py:112–131`; no leader lock anywhere  
...
- **Severity:** high  
- **Score:** 9  
- **Failure scenario:** `tradeSummary` circuit opens. `_poll_prices` logs and returns `[]`. `run_once` still sets `last_tick_ok = True`. Ops sees `/health` 200. In `python -m koel poller`, `HealthState` is never updated at all (always `ok=True`, empty details). Deploy looks healthy while alerts are frozen.  
- **Where:** `koel/poller.py:74–84`, `95–102`; `koel/__main__.py:82–103` vs `56–70`; test even asserts OK on circuit open (`tests/test_poller_resilience.py:60`)  
...
- **Where:** `koel/config.py:41–42`, `koel/poller.py:149–170`, `.env.example`  
...
- **Where:** `koel/adapters/cse.py:289–298`  
...
- **Where:** `koel/bot.py:24–28`, `65–92`; `CLAUDE.md:93–100`  
...
- **Where:** `koel/adapters/cse.py:386–391`, `koel/bot.py:79–84`, `164–168`  
...
- **Where:** `koel/adapters/cse.py:186`, `docs/endpoint_probe_report.md:193`  
...
- **Where:** `db/migrations/001_initial.sql:79–81`, `koel/storage.py:271–289`, `koel/bot.py:175`  
...
- **Where:** `koel/__main__.py:50–79`, `koel/health.py:49–52`, `koel/config.py:46–47`  
...
- **Where:** `koel/poller.py:186–192`, `koel/storage.py:378–394`  
...
| 6. Code quality | **Pass-ish.** Types/pydantic present; no TODOs; dead `if not disclosure_rules: pass`; pytest cov gated only on `koel.rules`. |
