# koel brand assets

Product name: **koel** (lowercase). Wordmark: lowercase **koel**. Mark: capital **K**.

Source design: [Canva — Koel rebrand to Koel](https://canva.link/mofjxofiv0cyhh8)
(`branding/canva/` holds downloaded page previews).

| File | Use |
|---|---|
| `koel-logo.svg` | Square black master wordmark |
| `koel-mark.svg` | Square black master K mark |
| `canva/` | Raw Canva page PNGs/WebPs |

Ink: `#111111`. Web runtime copies: `web/public/brand/koel-*` (transparent UI crops).
Use via `KoelWordmark` / `KoelMark` / `KoelLockup` in `web/src/components/brand/koel-brand.tsx`.

Third-party marks used in marketing chrome:

| File | Source |
|---|---|
| `web/public/brand/telegram-mark.svg` | Brandfetch `telegram.org` symbol (circular mark) |
| `web/public/brand/telegram-mark.png` | Same mark, raster |
