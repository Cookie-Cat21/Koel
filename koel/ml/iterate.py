...
from koel.ml.diagnose import PredRow, analyze_rows, load_sector_map
...
def _enrich_cross_section(samples: list[Sample]) -> list[Sample]:
    ...
    for day_samples in by_day.values():
        ...
        def pct_map(index: int, samples: list[Sample] = day) -> dict[str, float]:
            pairs = [
                (s.symbol, s.x[index])
                for s in samples
            ...
            if len(pairs) < 3:
                return {s.symbol: float("nan") for s in samples}
